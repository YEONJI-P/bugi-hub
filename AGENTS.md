...
이 저장소는 Bugi Hub의 공개 Next.js 애플리케이션이다. 현재 구성, 실행, 상태 감시와 이미지 계약은 `README.md`가 정본이다.
...
- Java 애플리케이션, Compose, nginx, TLS와 인프라 자동화는 이 저장소에 추가하지 않는다.
...
- health 응답 계약은 HTTP 200과 `{"status":"UP"}`이다. DB health는 미설정 또는 연결 실패 시 HTTP 503을 반환한다.

## 환경과 비밀값

- `.env`, credential, 인증서, 토큰과 실제 비밀값은 읽거나 추적하거나 이미지에 복사하지 않는다.
- 새 환경변수는 실제 서버 소비 코드, `.env.example`, `README.md`의 이름을 함께 맞춘다.
...
Dockerfile이나 런타임 경계를 바꾸면 이미지를 빌드하고 `/actuator/health`, non-root 사용자와 Docker health 상태를 함께 확인한다.
