...
export const workLogs: WorkLogEntry[] = [
  ...
  },
  {
    ...
    story: {
      ...
      cause: "외부 사용자가 접근할 경로와 컨테이너 내부 운영 경로의 요구가 다르고, 일반 이미지 교체만으로는 데이터베이스 변경의 복구 지점을 보장할 수 없었습니다. 스키마 이력은 운영 데이터가 쌓이기 전에 Flyway로 도입하고 운영 프로파일은 validate로 잠가 둔 상태였습니다.",
  ...
  },
  {
    ...
    story: {
      type: "decision",
      background: "릴리스마다 이미지 확인과 홈서버 교체를 손으로 반복했고, 배포 비밀값은 GitHub에 올리지 않는다는 전제도 지켜야 했습니다.",
      options: "GitHub Actions가 홈서버에 직접 배포하는 push 방식과, 홈서버가 저장소 변경을 감지해 스스로 갱신하는 pull 방식을 비교했습니다.",
      decision: "비밀값이 서버 밖으로 나가지 않는 pull 방식을 선택했습니다. CI를 통과한 commit SHA 이미지를 자동 PR로 제안받되, env·migration 검토와 병합은 사람이 승인합니다.",
      outcome: "병합 후 홈서버 timer가 해당 SHA로 컨테이너만 교체하는 전체 경로를 실제 릴리스로 확인했고, 검토가 필요한 변경은 승인 단계에서 걸러집니다.",
    },
    tags: ["GitHub Actions", "Deployment", "CI/CD"],
  },
  {
    slug: "channel-threshold-contract",
    date: "2026-07-19",
    category: "Service Integration",
    repository: "sensor-monitor",
    title: "임계값·방향 계약을 세 층에서 강제",
    summary: "임계값과 방향이 따로 저장될 수 있던 문제를 화면·서비스 검증과 DB CHECK 제약으로 함께 막았습니다.",
    story: {
      type: "solution",
      problem: "임계값만 있고 판정 방향이 없는 채널이 만들어질 수 있어, 이상 판정이 채널마다 다르게 해석될 여지가 있었습니다.",
      cause: "입력 검증이 화면에만 있어 API 직접 호출이나 이미 저장된 데이터에는 같은 규칙이 적용되지 않았습니다.",
      decision: "임계값과 방향은 둘 다 있거나 둘 다 없어야 한다는 계약을 정했습니다. 화면·서비스 검증에 더해, 기존 migration은 그대로 두고 새 V5에서 legacy 부분 입력을 정규화한 뒤 CHECK 제약으로 DB에서도 강제했습니다.",
      outcome: "어떤 경로로 저장해도 같은 임계 계약이 적용되고, 방향별 판정과 설명 문구도 계약과 일치하게 됐습니다.",
    },
    tags: ["PostgreSQL", "Flyway", "Validation"],
  },
  {
    slug: "public-routing",
    date: "2026-07-18",
